...
lib.adapter_forward_batch.argtypes = [
    np.ctypeslib.ndpointer(dtype=np.float32, flags='C_CONTIGUOUS'),
    np.ctypeslib.ndpointer(dtype=np.float32, flags='C_CONTIGUOUS'),
    ctypes.c_int
]
lib.adapter_forward_batch.restype = None
...
lib.test_adapter.argtypes = []
lib.test_adapter.restype = None

# 初始化适配器
weight_dir = b"/root/千问白盒化实验/c_adapter"
print(f"初始化适配器，权重目录: {weight_dir.decode()}")

ret = lib.adapter_init(weight_dir)
if ret != 0:
...
rel_diff = abs_diff / (np.abs(output_torch) + 1e-8)
...
batch_output_c = np.zeros((batch_size, output_dim), dtype=np.float32)

# C批量前向
start = time.perf_counter()
lib.adapter_forward_batch(batch_input, batch_output_c, batch_size)
c_batch_time = (time.perf_counter() - start) * 1000  # ms per batch

print(f"C适配器批量处理 ({batch_size}个样本): {c_batch_time:.4f} ms")
print(f"  平均每个样本: {c_batch_time/batch_size:.4f} ms")

# PyTorch批量前向
batch_torch = torch.from_numpy(batch_input).float()
start = time.perf_counter()
with torch.no_grad():
    output_torch_batch = pytorch_adapter(batch_torch)
torch_batch_time = (time.perf_counter() - start) * 1000

print(f"PyTorch批量处理 ({batch_size}个样本): {torch_batch_time:.4f} ms")
print(f"  平均每个样本: {torch_batch_time/batch_size:.4f} ms")

batch_speedup = torch_batch_time / c_batch_time if c_batch_time > 0 else 0
print(f"  批量加速比: {batch_speedup:.2f}倍")
...
result = {
    ...
    "numerical_validation": {
        ...
        "max_relative_error": float(max_rel_diff),
    ...
    "performance": {
        ...
        "batch_pytorch_ms": float(torch_batch_time),
        "batch_c_ms": float(c_batch_time),
