...
def extract_hidden_state_strategy1(model, tokenizer, text):
    """策略1: 第一个token的hidden_state (原始策略)"""
...
def extract_hidden_state_strategy4(model, tokenizer, text):
    """策略4: 最后4层的加权平均"""
    inputs = tokenizer(text, return_tensors="pt", truncation=True, max_length=512)
    inputs = {k: v.to(model.device) for k, v in inputs.items()}
    ...
    with torch.no_grad():
        ...
        # 加权平均：越靠近输出层权重越高
        weights = torch.tensor([0.1, 0.2, 0.3, 0.4], device=model.device).view(1, 4, 1)
        weighted_sum = torch.stack(last_layers, dim=1) * weights
        hidden_state = weighted_sum.sum(dim=1).mean(dim=1).float().cpu().numpy()
    
    return hidden_state

def extract_hidden_state_strategy5(model, tokenizer, text):
    """策略5: 使用注意力权重的加权平均"""
    inputs = tokenizer(text, return_tensors="pt", truncation=True, max_length=512)
    inputs = {k: v.to(model.device) for k, v in inputs.items()}
    ...
    with torch.no_grad():
        ...
        # 取第一个注意力头的注意力权重（平均所有头）
        attentions = outputs.attentions[-1]  # [batch, heads, seq_len, seq_len]
        avg_attention = attentions.mean(dim=1)  # [batch, seq_len, seq_len]
        
        # 使用注意力权重加权平均hidden_state
        # 取第一个token对其他所有token的注意力作为权重
        attention_weights = avg_attention[:, 0, :]  # [batch, seq_len]
        attention_weights = attention_weights.unsqueeze(-1)  # [batch, seq_len, 1]
        
        weighted_hidden = (last_hidden * attention_weights).sum(dim=1)
        hidden_state = weighted_hidden.float().cpu().numpy()
...
def main():
    ...
    strategies = [
        ...
        (extract_hidden_state_strategy4, "策略4: 最后4层加权"),
        (extract_hidden_state_strategy5, "策略5: 注意力加权"),
    ]
    ...
    output_file = "/root/千问白盒化实验/logs/phase1_strategies_results.json"
    ...
    if best_strategy:
        ...
        best_name = best_strategy["strategy_name"]
        
        print(f"最佳策略: {best_name}")
        print(f"最佳判别性分数: {best_disc:.4f}")
        
        if best_disc > 0.4:
            print("🎉 优秀！找到有效的hidden_state提取策略")
            print("   建议：使用此策略继续阶段二实验")
        elif best_disc > 0.2:
            print("✅ 良好！hidden_state有足够判别性")
            print("   建议：可以继续实验，使用此策略")
        elif best_disc > 0.1:
            print("⚠️ 一般！判别性较弱但可用")
            print("   建议：考虑结合适配器训练")
        elif best_disc > 0:
            print("❌ 较弱！判别性不足")
            print("   建议：需要训练适配器来增强特征提取")
        else:
            print("❌ 很差！所有策略都无效")
            print("   建议：重新设计实验方案或直接训练适配器")
    else:
        print("❌ 所有策略都失败")
        print("   建议：直接开始适配器训练")
