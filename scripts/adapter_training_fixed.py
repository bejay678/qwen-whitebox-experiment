...
class FactDataset(Dataset):
    ...
    def __init__(self, data_path, tokenizer, model, max_samples=200):
        ...
        print(f"加载数据: {len(self.training_pairs)}个训练对")
        ...
        if self.hidden_states:
            ...
            print(f"涉及事实: {len(set(self.fact_ids))}个不同事实")
        else:
            print("❌ 没有提取到有效的hidden_state")
    ...
    def build_pairs(self):
        ...
        print(f"构建完成: {len(self.positive_pairs)}个正样本对, {len(self.negative_pairs)}个负样本对")
...
def evaluate_adapter(adapter, dataset, device):
    """评估适配器性能"""
    if len(dataset.hidden_states) == 0:
        return None
